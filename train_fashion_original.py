...
# model
# att_default = ["无袖", "短袖", "盖肩袖", "中袖/5分袖", "6分袖/7分袖/8分袖", "9分袖", "长袖", "红色", "粉色", "橙色", "黄色", "绿色", "蓝色", "紫色", "灰色",
        ...
        with pylib.Timer(is_output=False) as t:
            ...
            if (it + 1) % 1000 == 0:
                save_path = saver.save(sess,
                                       '%s/Epoch_(%d)_(%dof%d).ckpt' % (ckpt_dir, epoch, it_in_epoch, it_per_epoch))
            ...
            if (it + 1) % 70 == 0:
                ...
                im.imwrite(im.immerge(sample, n_sample, 1),
                           '%s/Epoch_%d_%dof%d.jpg' % (save_dir, epoch, it_in_epoch, it_per_epoch))
except:
    traceback.print_exc()
finally:
